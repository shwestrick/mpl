  ...
  uint32_t desired_depth)
{
  ...
  uint64_t desired_top = PACK_TAGIDX(local_top_tag, desired_depth);

  if (local_top_idx != local_bot) {
    DIE("Bug! Attempt to set depth of non-empty deque!");
  }
  ...
  if (desired_depth == local_bot) {
    ...
    __atomic_store_n(bot, desired_depth, __ATOMIC_RELEASE);
    __atomic_store_n(top, desired_top, __ATOMIC_RELEASE);
  }
  else {
    __atomic_store_n(top, desired_top, __ATOMIC_RELEASE);
    __atomic_store_n(bot, desired_depth, __ATOMIC_RELEASE);
