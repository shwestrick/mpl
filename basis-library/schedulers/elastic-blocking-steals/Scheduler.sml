      ...
      | SOME c =>
          let
            ...
            val packedState' = packState
              { rejecting = false
              , priority = priority
              , firstChild = (if c' < 0 then NONE else SOME c')
              }
          in
            if not (casRef (state p) (packedState, packedState')) then
              tryWakeOneChildOf p
            else
              (* successfully removed the first child, so now just wake em up *)
              ( MLton.Parallel.takeSleepLock c
              ; next c := ~2
              ; MLton.Parallel.signalSleepLock c
              ; MLton.Parallel.releaseSleepLock c
  ...
  struct
    ...
    fun parfork thread depth (f : unit -> 'a, g : unit -> 'b) =
      let
        ...
        val _ = push g'
        val _ = tryWakeOneChildOf (myWorkerId ())
              ...
              in
                ...
                | NONE =>
                    if tryChill friend then
                      case trySteal friend of
                        SOME (task, depth) => (task, depth, tickTimer idleTimer)
                      | NONE => loop 0 (tickTimer idleTimer)
                    else
